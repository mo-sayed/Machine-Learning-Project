...
# df_train %>% transmute(SalePrice, MSSubClass = as.factor(MSSubClass)) %>% 
#   mutate(MSSubClass = fct_reorder(MSSubClass, SalePrice, .fun = 'median')) %>% 
#   ggplot() +geom_boxplot(aes(x=MSSubClass, y = SalePrice))
...
# df_train %>% select(c(SalePrice, MSZoning)) %>%  
#   mutate(MSZoning = fct_reorder(MSZoning, SalePrice, .fun = 'median')) %>% 
#   ggplot() +geom_boxplot(aes(x=MSZoning, y = SalePrice))
...
df_train %>% filter(is.na(LotFrontage)) %>% group_by(LotFrontage) %>% summarise(n())   # 259 missing values
# Impute LotFrontage values
df_train$LotFrontage = kNN(df_train %>% select(c(LotArea, Neighborhood, LotFrontage)), k=5)$LotFrontage
# Remove outliers
df_train = df_train %>% filter(LotFrontage < 300)   # Outlier IDs --> 935 & 1299
# Check if predictor is has linear relationship with SalePrice
...
# df_train %>% select(c(SalePrice, LotArea)) %>% ggplot() +geom_point(aes(x=log(LotArea), y = SalePrice)) # Linear relationship for log(LotArea) with SalePrice
# Remove outliers
df_train = df_train %>% filter(!Id %in% c(250, 314, 336, 707))    # Ids --> 250, 314, 336, 707
df_train %>% filter(SalePrice > 600000, LotArea < 30000) %>% select(c(Id, SalePrice, HouseStyle, LotArea, Neighborhood, BldgType))
df_train = df_train %>% filter(!Id %in% c(692, 1183))    # Ids --> 692, 1183   are outliers
# Check for missing values
df_train %>% filter(is.na(LotArea)) %>% group_by(LotArea) %>% summarise(n())   # 0 missing values
# Check if predictor  has significant linear relationship with SalePrice
...
summary(model_Alley)
# Looking at the frequency of observations for different Alley levels, it doesn't provide any
# signifance for the SalePrice; hence it is dropped from the model
...
# IRR2 & IRR3 have only 41 entries out of 1460 and they have similar distribution and also
# have significantly higher mean/median than Reg
...
# Used  plot to see what neighbourhoods can be grouped together
df_train = df_train %>% mutate(Neighborhood = ifelse(Neighborhood %in% c("BrDale","IDOTRR","MeadowV"), "A", 
                            ifelse(Neighborhood %in% c("BrkSide","Edwards","OldTown"), "B",
                           ifelse(Neighborhood %in% c("Blueste","Mitchel","NAmes","NPkVill","SWISU","Sawyer"), "C",
                  ifelse(Neighborhood %in% c("Blmngtn","ClearCr","CollgCr","Crawfor","Gilbert","NWAmes","SawyerW"), "D", 
                  ifelse(Neighborhood %in% c("Somerst", "Timber", "Veenker" ), "E", "F"))))))
# Grouped neighborhoods into 6 different groups/levels, based on their median and std
df_train %>% group_by(Neighborhood) %>% summarise(n())
...
df_train %>% group_by(BldgType) %>% summarise(avg_ = mean(SalePrice), med_ = median(SalePrice), sd_ = sd(SalePrice))
...
# Group levels based on their median and spread
df_train = df_train %>%  mutate(BldgType = ifelse((BldgType %in% c("2fmCon","Duplex","Twnhs")), "C", 
                                                   ifelse(BldgType == "1Fam", "A", "B")))
...
df_train %>% group_by(HouseStyle) %>% summarise(med_ = median(SalePrice),sd_ = sd(SalePrice), avg_ = mean(SalePrice))
df_train %>% group_by(HouseStyle) %>% summarise()
...
# Group levels based on their median and spread
df_train = df_train %>% mutate(HouseStyle = ifelse(HouseStyle %in% c("1.5Unf", "1.5Fin", "2.5Unf", "SFoyer"), "A",
                                        ifelse(HouseStyle %in% c("1Story", "SLvl"), "B", "C")))
...
df_train %>% filter(is.na(OverallQual)) %>% summarise(n())
...
#   ggplot() + geom_point(aes(x = as.factor(YearBuilt), y = (SalePrice)))
...
df_train = df_train %>% mutate(YrDiffSoldBuilt = (as.numeric(YrSold) - as.numeric(YearBuilt)))
...
df_train$YrDiffRemodBlt = df_train$YearRemodAdd - df_train$YearBuilt
df_train$YrDiffSoldRemod = df_train$YrSold - df_train$YearRemodAdd
...
model_RoofStyle = lm(SalePrice ~  RoofStyle, data = df_train)
summary(model_RoofStyle)
...
df_train = df_train %>% mutate(Exterior1st = ifelse(Exterior1st %in% c("BrkComm", "AshpShn", "CBlock", "PreCast", "AsbShng"), "A",
...
df_train = df_train %>% mutate(Exterior2nd = ifelse(Exterior2nd %in% c("Brk Cmn", "AshpShn", "CBlock", "PreCast", "AsbShng"), "A",
...
# the model seems significant with a low p-value and all levels are not significant
# Keep

#  ----------------------------------------------------------------------------------
# MasVnrType
# df_train %>% select(c(SalePrice, MasVnrType)) %>% 
#   mutate(MasVnrType = fct_reorder(MasVnrType, SalePrice, .fun = 'median')) %>% 
#   ggplot()+ geom_boxplot(aes(x=MasVnrType, y = SalePrice))
df_train %>% group_by(MasVnrType) %>% summarise(n())
# Impute NAs
df_train$MasVnrType = kNN(df_train %>% select(c(BldgType, HouseStyle, Exterior1st, Exterior2nd, Neighborhood, MasVnrType)), k=10)$MasVnrType
...
# check for missing values and impute the NAs
df_train %>% filter(is.na(MasVnrArea)) %>% select(c(SalePrice, MasVnrArea, MasVnrType, Id))
df_train$MasVnrArea = kNN(df_train %>% select(c(BldgType, HouseStyle, Exterior1st, Exterior2nd, Neighborhood, MasVnrType, MasVnrArea)), k=10)$MasVnrArea
df_train %>%  filter( Id %in% c(651, 937, 974, 978, 1244, 1279)) %>% select(c(SalePrice, MasVnrArea, MasVnrType, Id))

model_MasVnrArea = lm(SalePrice ~ MasVnrArea, data = df_train)
summary(model_MasVnrArea)
# Keep as a predictor
...
# df_train = df_train %>% mutate(Foundation = (ifelse(((Foundation == "Wood")|(Foundation == "Stone")), "C", 
#                                                     ifelse(((Foundation == "Slab") |(Foundation == "BrkTil")|(Foundation == "CBlock")), "A","B"))))

df_train = df_train %>% mutate(Foundation = ifelse(Foundation == "PConc", "A", "B"))
...
# Impute NA values
df_train$BsmtQual = kNN(df_train %>% select(c(SalePrice,OverallQual, Foundation, BsmtFinSF1, TotalBsmtSF, BsmtFinSF2, BsmtQual)), k=10)$BsmtQual
...
df_train$BsmtCond = kNN(df_train %>% select(c(BsmtQual, OverallQual, Foundation, BsmtFinSF1, TotalBsmtSF, BsmtFinSF2, BsmtCond)), k=10)$BsmtCond
...
df_train = df_train %>% mutate(BsmtExposure = ifelse(is.na(BsmtExposure), "NoBasement",
                              ifelse(BsmtExposure == "No", "No",
                              ifelse(BsmtExposure == "Mn", "Mn",
                              ifelse(BsmtExposure == "Av", "Av", "Gd")))))
# df_train$BsmtExposure = kNN(df_train %>% select(c(SalePrice, BldgType, LandSlope, LotConfig, LandContour ,Foundation, BsmtFinSF1, TotalBsmtSF, BsmtQual, BsmtExposure)), k=10)$BsmtExposure
...
df_train = df_train %>% mutate(BsmtFinType1 = ifelse(is.na(BsmtFinType1), "No Basement", 
                                                     ifelse(BsmtFinType1 == "GLQ", "A", "B")))
df_train = df_train %>% mutate(BsmtFinType2 = ifelse(is.na(BsmtFinType2), "No Basement", 
...
# Model has a somewhat low p-value but predictors are not significant
# Remove from the model
...
model_BsmtUnfSF = lm(SalePrice ~ (BsmtUnfSF) , data = df_train)
summary(model_BsmtUnfSF)
# Model has LOW p-value, BUT LOW R^2 so doesn't explain the variance of the model
...
# Impute BsmtFinSF1 & BsmtFinSF2
df_train$BsmtFinSF1 = kNN(df_train %>% select(c(SalePrice, BldgType, TotalBsmtSF, BsmtQual, BsmtExposure, BsmtFinType1, BsmtFinSF1)), k=10)$BsmtFinSF1
df_train$BsmtFinSF2 = kNN(df_train %>% select(c(SalePrice, TotalBsmtSF, BsmtQual, BsmtExposure, BsmtFinType1, BsmtFinSF1, BsmtFinSF2)), k=10)$BsmtFinSF2


# ------------------------------------------------------------------------------------------------
# df_train %>% filter(is.na(BsmtFinSF1) | (BsmtFinSF1 == 0 & BsmtFinType1 != "Unf")) %>% summarise(n())
# df_train %>% filter(is.na(BsmtFinSF1) | (BsmtFinSF1 == 0 & BsmtFinType1 != "Unf")) %>% select(c(Id, BsmtFinSF1,BsmtFinType1, BsmtFinSF2,BsmtFinType2, TotalBsmtSF, BsmtUnfSF))
# 
...
df_train %>% group_by(Electrical) %>% summarise(n(), mean(YearBuilt),sd(SalePrice),median(YearBuilt))
...
df_train$Electrical = kNN(df_train %>% select(c(YearBuilt, SalePrice, Electrical)), k=10)$Electrical
# Bin 'FuseF' and 'FuseP' together as 'FuseB' for Bad Fuse
df_train = df_train %>% mutate(Electrical = ifelse(Electrical == "SBrkr", "A", "B"))
...
df_train %>% filter(X1stFlrSF > 3000) %>% select(c(Id, SalePrice, X1stFlrSF))
# Remove outlier Id 524 with high 1st floor SF and very low price
df_train = df_train %>% filter(Id != 524)
...
df_train %>% select(c(SalePrice, BsmtFullBath)) %>% 
  ggplot() +geom_boxplot(aes(x=as.factor(BsmtFullBath), y = SalePrice))
...
df_train$BsmtFullBath = kNN(df_train %>% select(c(BsmtFinSF1, TotalBsmtSF, BsmtExposure, FullBath, TotRmsAbvGrd, BsmtFullBath )), k=10)$BsmtFullBath
df_train$BsmtHalfBath = kNN(df_train %>% select(c(BsmtFinSF1, TotalBsmtSF, BsmtExposure, FullBath, HalfBath, TotRmsAbvGrd, BsmtHalfBath )), k=10)$BsmtHalfBath
df_train %>% filter(is.na(BsmtFullBath) | is.na(BsmtHalfBath)) %>% select(Id)
...
df_train = df_train %>% filter(TotRmsAbvGrd != '14')
...
model_Functional = lm(SalePrice ~ Functional, data = df_train)
...
df_train %>% select(c(SalePrice, Fireplaces)) %>% 
  ggplot() +geom_boxplot(aes(x=as.factor(Fireplaces), y = SalePrice))
df_train %>% group_by(Fireplaces) %>% summarise(n())

# df_train$Fireplaces = as.integer(df_train$Fireplaces)
for (i in (1:nrow(df_train))){
  df_train$Fireplaces[i] = (ifelse(df_train$Fireplaces[i] == 0, "No", "Yes"))
}
...
# Remove from model as there are too many NAs and can't impute properly
...
df_train$GarageYrBlt = kNN(df_train %>% select(c(YearBuilt, YearRemodAdd, GarageType, GarageCond, GarageQual, GarageYrBlt)), k=10 )$GarageYrBlt

df_train$YrDiffGarageBlt = df_train$GarageYrBlt - df_train$YearBuilt
...
df_train %>% select(c(SalePrice, GarageCars)) %>% 
  ggplot() +geom_boxplot(aes(x=as.factor(GarageCars), y = SalePrice))
df_train %>% group_by(GarageCars) %>% summarise(n())
# df_train %>% filter(GarageCars == 4) %>% select(c(SalePrice, LotArea, BedroomAbvGr, Id))
...
df_train = df_train %>% mutate(FrontAreaSF = WoodDeckSF + OpenPorchSF + EnclosedPorch + X3SsnPorch + ScreenPorch)
# df_train %>% transmute(AreaSF = WoodDeckSF + OpenPorchSF + EnclosedPorch + X3SsnPorch + ScreenPorch, SalePrice) %>% 
#   ggplot() + geom_point(aes(x = AreaSF, y = SalePrice))

# df_train = df_train %>% mutate(WoodDeckSF = ifelse(WoodDeckSF == 0, 0.01, WoodDeckSF))

model_PavedDrive = lm(SalePrice ~ (WoodDeckSF) , data = df_train)
summary(model_PavedDrive)
# WoodDeckSF linear model has a low p-value,  R^2 and factors p-value are somewhat significant
# Remove from model as it has many 0 values that can't be imputed and they distort the distribution of SalePrice
...
model_OpenPorchSF = lm(SalePrice ~ (OpenPorchSF) , data = df_train)
...
model_EnclosedPorch = lm(SalePrice ~ (EnclosedPorch) , data = df_train)
...
# Created new variable "SoldAfterBuilt" from YrSold and YearBuilt
...
df_train = df_train %>% mutate(SaleType = ifelse(SaleType %in% c("Oth", "ConLI", "COD", "ConLD", "ConLW"), "A",
...
df_train = df_train %>% select(-c('Street', 'Alley', 'Utilities', 'LotConfig', 'LandSlope', 'Condition1', 'YearBuilt',
...
# View(df_train %>% sapply(function(x) sum(is.na(x))))
